---
title: "Final Project"
author: "Gabe, Chisom, Priya, Jaegoo, Harin"
date: "2025-04-24"
output: pdf_document
---
...
```{r median-analysis, messages=FALSE}
...
#Graph Findings 
hist(sai_drug$anxious, main = "Anxious Median - DRUG",
     xlab = "Anxiety Levels")
abline(v = med_drug, col="blue")
hist(sai_placebo$anxious, main = "Anxious Median - PLACEBO",
     xlab = "Anxiety Levels")
...
for (reps in 1:10000) {
  ...
  resample_diff <- outer(drug_sample, placebo_sample, `-`)
  
  # Median of all resampled differences
...
hist(boot_med_diff, 
     breaks = 50,
     main = "Anxiety Medians Bootstrapping",
...
hist(sai_drug$anxious, main = "Anxious Mean - DRUG", xlab = "Anxiety Levels")
abline(v = mean_drug, col = "purple")

hist(sai_placebo$anxious, main = "Anxious Mean - PLACEBO", xlab = "Anxiety Levels")
...
In filtering our data to only the caffeine and placebo groups, we misrepresent the number of participants and therefore, our findings are skewed. With 600 observations in sai_drug and 157 observations in sai_placebo, The data collected for each is imbalanced. Additionally, the placebo group also includes people who could have taken caffeine drug, but their emotional measures showed no variation. Therefore, the results from this data have no external validity because the data was manipluated from the start. Below we wanted to show a hypothetical scenario that reflects the implications of lying about such data. Let's say an energy drink manufacturer is planning to promote their product as "incapable of causing anxiousness associated with caffeine over-consumption." To support this claim, they also run the same experimental results from sai with some 'tweaks'.
...
abline(v = mislead_med_diff, col = "blue", lwd = 2)
...
Both visually and through how the median differences were calculated here, this beverage company sells the idea that anxiety levels of their product are the same as those who do not drink caffeinated products. This benefits the company to lie because it sells the idea that their product has no anxious harm.